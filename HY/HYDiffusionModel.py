...
def extract(a, t, x_shape):
    # (100)；t：[non,]；[non, 1, 3]
    b, *_ = t.shape # b:non
    out = a.gather(-1, t) # 让每个实体的扩散步数不同
    return out.reshape(b, *((1,) * (len(x_shape) - 1))) # [non,1,1]
...
class GaussianDiffusion_ST(nn.Module):
    def __init__(
        ...
        *,
        ...
        register_buffer = lambda name, val: self.register_buffer(name, val.to(torch.float32)) # 在PyTorch模型中注册额外的缓冲区（buffer），这些缓冲区不会像模型的参数一样通过反向传播进行更新，但它们会被优化器正确地传输到所指定的设备（如GPU或CPU）

        register_buffer('betas', betas) # name（缓冲区的名称）和val（要注册的张量）
    ...
    def _vb_terms_bpd(self, x_start, x_t, t, *, clip_denoised: bool,cond=None):
        # x_start：[non, 1, 3]；x_t：[non, 1, 3]；t：[non,]；
        # clip_denoised：True；
        # cond：[non,1,d_model*3]
    ...
    def q_sample(self, x_start, t, noise=None):
        # # x_start：[non, 1, 3]；t：[non,]；noise：[non, 1, 3]
        ...
        return (
            ...
            # # [non,1,1]*[non, 1, 3]+[non, 1, 1]*[non, 1, 3]
        )
    ...
    def p_losses(self, x_start, t, noise = None, cond=None):
        # x_start（img）:[non, 1, 3]；t:[non,]；cond:[non,1,d_model*3]
        s0 = time.time()
        b, c, n = x_start.shape
        '''1.初始化噪声'''
        noise = default(noise, lambda: torch.randn_like(x_start)) # 初始化标准正态分布的噪声，shape为x_start的形状
        '''2.前向扩散过程''' # x_start：[non, 1, 3]；t：[non,]；noise：[non, 1, 3]--》[non, 1, 3]
        x = self.q_sample(x_start = x_start, t = t, noise = noise)
        # "self-conditioning"（自条件化）是一个概念，它指的是在模型生成或预测过程中，利用自身生成的部分结果作为条件来指导后续的生成或预测步骤
        x_self_cond = None
        if self.self_condition and random() < 0.5: # 随机做出决策
            with torch.no_grad():
                # x:[non, 1, 3]；t:[non,]
        ...
        s1 = time.time()
        e1 = time.time()
        # x:[non, 1, 3]；t:[non,]；x_self_cond = None；cond:[non,1,d_model*3]--》 [non,1,3]
        model_out = self.model(x, t, x_self_cond,cond)
        if self.objective == 'pred_noise':
            target = noise # noise：[non, 1, 3]
        elif self.objective == 'pred_x0':
            target = x_start
        elif self.objective == 'pred_v':
        ...
        if self.loss_type in  ['l1','l2']:
            # [non, 1, 3] 、[non, 1, 3]
            loss = self.loss_fn(model_out, target, reduction = 'none') # [non, 1, 3]
        elif self.loss_type == 'Euclid':
            loss = self.loss_fn(model_out, target)
        loss = reduce(loss, 'b ... -> b (...)', 'mean') # [non,3]
        loss = loss * extract(self.p2_loss_weight, t, loss.shape) # [timesteps]，t:[non,]，[non, 3]--> [non,1];[non,3]*[non,1]=[non,3]
        e0 = time.time()
        return loss.mean()
    ...
    def NLL_cal(self, x_start, cond, noise=None, clip_denoised=True):
        # [time_non, 1, 3],[time_non,1,d_model*3]
        """
        Compute the entire variational lower-bound, measured in bits-per-dim,as well as other related quantities. 变分下限
        :param model: the model to evaluate loss on. 要评估损失的模型
        :param x_start: the [N x C x ...] tensor of inputs.
        :param clip_denoised: if True, clip denoised samples. 如果为true，剪辑去噪样本
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning. 如果不是None，则要传递给模型的额外关键字参数
        :return: a dict containing the following keys:
                 - total_bpd: the total variational lower-bound, per batch element.每个批次的总变分下限
                 - prior_bpd: the prior term in the lower-bound. 下限值中的先验项
                 - vb: an [N x T] tensor of terms in the lower-bound. 下限中的项的[N*T]张量  变分下界
                 - xstart_mse: an [N x T] tensor of x_0 MSEs for each timestep. 每步的x0MSE张量[N*T]
                 - mse: an [N x T] tensor of epsilon MSEs for each timestep. 每步的epsilon张量[N*T]
        """
        # [time_non, 1, 3]
        x_start = normalize_to_neg_one_to_one(x_start)
        device = x_start.device
        batch_size = x_start.shape[0] # non

        vb_all, vb_temporal_all, vb_spatial_all = [], [], []

        for t in list(range(self.num_timesteps))[::-1]: # 反转遍历
            t_batch = torch.tensor([t] * batch_size, device=device)
            # [time_non, 1, 3],t_batch：[non,]-->[non, 1, 3]
            x_t = self.q_sample(x_start=x_start, t=t_batch, noise=noise)
            # Calculate VLB term at the current timestep
            with torch.no_grad():
                vb, vb_temporal, vb_spatial, _ = self._vb_terms_bpd(
                    x_start=x_start, #[non, 1, 3]
                    x_t=x_t, # [non, 1, 3]
                    t=t_batch, # [non,]
                    clip_denoised=clip_denoised, # True
                    cond = cond # [non,1,d_model*3]
                )
            vb_all.append(vb.unsqueeze(dim=1))
            vb_temporal_all.append(vb_temporal.unsqueeze(dim=1))
            vb_spatial_all.append(vb_spatial.unsqueeze(dim=1))
    ...
    def forward(self, img, cond, *args, **kwargs):
        ...
        # img:[non, 1, 3],cond:[non,1,d_model*3]
        # non,1,3,cpu/gpu,1+2
        b, c, n, device, seq_length, = *img.shape, img.device, self.seq_length
        assert n == seq_length, f'seq length must be {seq_length}'
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long() # 生成一个维度为(non,)的整数张量，相当于为每个事件生成了一个
        img = normalize_to_neg_one_to_one(img)
        return self.p_losses(img, t, cond=cond,  *args, **kwargs)  # img:[non, 1, 3]；t:[non,]；cond:[non,1,d_model*3]
...
class ST_Diffusion(nn.Module):
    ...
    def forward(self, x, t, x_self_cond = None, cond = None):
        # x:[non, 1, 3]；t:[non,]；x_self_cond = None；cond:[non,1,d_model*3]
        # x_spatial：[non,1,2],x_temporal:[non,1,1]
        x_spatial, x_temporal = x[:, :, 1:].clone(), x[:, :, :1].clone()

        hidden_dim = int(cond.shape[-1] / 3)  # d_model

        # h_i-1 enc_out_non_mask
        # [non,1,d_model],[non,1,d_model],[non,1,d_model]
        cond_temporal, cond_spatial, cond_joint = cond[:, :, :hidden_dim], cond[:, :, hidden_dim:2 * hidden_dim], cond[:, :,2 * hidden_dim:]
        # cond编码 [non,1,num_units]
        cond = self.cond_all(cond)
        t_embedding = self.time_mlp(t).unsqueeze(dim=1)  # [non,num_units]--》[non,1,num_units]
        cond_all = torch.cat((cond, t_embedding), dim=-1)  # [non,1,num_units],[non,1,num_units]-->[non,1,num_units*2]
        # softmax通常用于获得一个概率分布，其中每个元素除以该维度上所有元素的指数总和。
        alpha_s = F.softmax(self.linear_s(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2)  # [non, 2, 1]
        alpha_t = F.softmax(self.linear_t(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2)  # [non, 2, 1]
        for idx in range(3):
            # t_embedding = embedding_layer(t).unsqueeze(dim=1)
            # idx：0,2,4 ；x_spatial：[non,1,2]；x_temporal:[non,1,1]
            x_spatial = self.linears_spatial[2 * idx](x_spatial)  # [non,1,num_units]
            x_temporal = self.linears_temporal[2 * idx](x_temporal)  # [non,1,num_units]
            assert x_spatial.shape == t_embedding.shape
            x_spatial += t_embedding  # [non,1,num_units]
            x_temporal += t_embedding  # [non,1,num_units]
            # [non,1,num_units]、[non,1,num_units]、[non,1,num_units]
            ...
            # [non,1,num_units]、[non,1,num_units]
        ...
        x_output = torch.cat((x_temporal, x_spatial), dim=1)  # [non,2,num_units]
        x_output_t = (x_output * alpha_t).sum(dim=1,
                                              keepdim=True)  # [non,2,num_units]*[non, 2, 1]-->[non,2,num_units]-->[non,1,num_units]
        x_output_s = (x_output * alpha_s).sum(dim=1,
                                              keepdim=True)  # [non,2,num_units]*[non, 2, 1]-->[non,2,num_units]-->[non,1,num_units]
        # [non,1,2]×，[non,1,3]√
...
class Model_all(nn.Module):
    def __init__(self, transformer, diffusion,strembedding,fusion_B_R):
        super(Model_all, self).__init__()
        self.transformer = transformer
        self.diffusion = diffusion
        self.strembedding = strembedding
        self.fusion_B_R = fusion_B_R

if __name__ == '__main__':
    model_test = ST_Diffusion(n_steps=100,dim=1+2,condition = True,cond_dim=64)
    diffusion_test = GaussianDiffusion_ST(
        model_test,
        loss_type="l2",
        seq_length=1+2,
        timesteps=100,
    ...
    # x: [non, 1, 3]；t: [non, ]；x_self_cond = None；cond: [non, 1, d_model * 3]
    x = torch.randn(size=(7,1,3))
    t = torch.randn(size=(7,))
    cond = torch.randn(size=(7,1,64*3))
    result = diffusion_test(img=x,cond=cond)
    print(result.shape)
